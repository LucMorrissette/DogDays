...
| **Watercraft boarding + paddling** | Facing a nearby `Watercraft` and pressing `InputAction.Confirm` triggers a short scripted hop for the player and follower into directional canoe seats; while seated, normalized movement input paddles the craft in 8 directions while the canoe sprite snaps to the dominant cardinal axis, the main player rides the rear seat, the companion rides the bow, side-view occupants stay layered inside the hull, travel is validated against a dedicated watercraft blocker set plus other watercraft hulls, and confirm/cancel only disembarks when a valid landing spot exists | Keeps canoe interaction in the same confirm/proximity model as other props while extending it into a simple, readable ride mechanic. |
| **Walkable prop overrides** | Props like docks can contribute walkable bounds that suppress blocked terrain underneath for on-foot actors without automatically permitting watercraft overlap | Lets bridges/docks sit over blocked water or collision layers without carving holes in the base map while keeping canoe travel blocked by solid surface props. |
...
| **Opening party availability** | New Game begins with only the player active; companion follow stays disabled until the opening bedroom return beat completes | Keeps party presentation aligned with the authored intro and makes companion presence derivable from quest state on load. |
...
| **Forest starter weapon unlock** | The forest hatchet/projectile starter loadout is gated by session progression state instead of being granted automatically on `Maps/WoodsBehindCabin`; new games and old saves default to locked until explicitly granted | Removes implicit weapon grants from zone entry while keeping the existing weapon systems available for later quest- or reward-driven unlocks. |
