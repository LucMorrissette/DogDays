...
| **Door interaction cues** | Front-door and shed interactions edge-trigger open/close sounds, while locked front doors trigger a distinct failed-entry rattle only when the player first presses into the blocked range | Preserves readable door feedback without audio spam while the player remains near a door. |
