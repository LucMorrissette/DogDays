...
| `WatercraftBoardSequence` | Screen-owned canoe ride controller for the player and follower. Consumes a nearby `Watercraft` interaction, runs delayed hop-in/hop-out tweens into directional front/rear canoe seats, paddles the craft with normalized 8-direction seated input while snapping the canoe art to the dominant cardinal axis, draws side-view occupants inside the hull, and only returns control to overworld movement when a valid disembark spot is available. |
...
| `QuestManager` | Owns quest runtime state for the current play session. Loads linear quest definitions, auto-starts configured quests, starts triggered quests from `GameEventBus` payloads, advances objective counters, exposes the currently active main quest, tracks which started quest is pinned to the HUD, and marks quests complete while surviving `GameplayScreen` replacement through `GameSessionServices`. |
| `QuestNpcDialogRegistry` | Stores authored per-quest NPC bark overrides as `DialogScript` instances keyed by quest id and NPC id, so screens can resolve story-reactive NPC dialog without hardcoding quest branches. |
| `MissingGnomeSequence` | Screen-owned cottage-exit sequencer for the follow-up missing-gnome story beat. Pulls Mom out of autonomous patrol, moves her toward the boys near the front door, queues the warning dialog, and emits a completion request that the screen converts into the quest-unlock story beat. |
| `SummerIntroSequence` | Screen-owned opener sequencer for the first summer-vacation story beat. Queues Mom/companion/bedroom dialog, moves actors through the shared scripted-actor contract, requests the cabin-to-bedroom `GameplayScreen` replacement, and emits a completion request that the screen converts into `StoryBeatReached`. |
...
| `IndoorNavigator` | Reusable graph-navigation component for indoor NPCs. Accepts an `IndoorNavGraph`, picks random destination nodes, computes A* routes, tracks progress through the route, pauses at destinations, repaths when the entity reports being stuck, and can `ResetFrom()` a scripted pose when autonomy resumes. Does not move the entity — exposes `CurrentTargetPosition` for the owner to steer toward. Any indoor NPC can compose this component for autonomous patrolling. |
