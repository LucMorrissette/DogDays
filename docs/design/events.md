...
| `StoryBeatReached` | `GameEventBus` (published by `GameplayScreen` via `SummerIntroSequence`) | `GameEvent` — `targetId` = story beat id, `amount` = 1 | An authored narrative beat resolves and should count toward story-driven quest progression, such as `summer_intro_complete`. |
