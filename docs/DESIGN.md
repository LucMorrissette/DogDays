...
| **Graph-patrolling NPC** | `MomNpc` navigates via `IndoorNavigator` component backed by `IndoorNavGraph` — delegates route selection, progress tracking, and repath to `IndoorNavigator`; handles movement and collision locally, and can be paused/posed through `IScriptControllableNpc` during authored sequences | Provides autonomous indoor NPC presence using authored navigation graphs instead of hardcoded waypoints while keeping cutscene control reusable across patrol NPCs. |
...
| **Decorative tile-object overlays** | TMX object layers can place non-`propType` tile objects that `TiledWorldRenderer` draws in a dedicated top-overlay pass | Supports editor-authored wall cutaways, room borders, and other non-colliding trim without creating one-off prop entities. |
...
| **Watercraft props** | TMX `propType` metadata can spawn dedicated `Watercraft` entities for editor-placed surface craft such as canoes; these props can expose facing-aware boarding interactions, seated paddling movement, dedicated watercraft blocker checks against solid surface props, and per-map restored runtime state | Establishes a focused world-object category for rideable craft without overloading generic decorative props. |
| **Prop state variants** | Distinct TMX `propType` values can select authored visual states of the same focused prop entity, and gameplay may swap between those authored states at runtime based on local context (for example, doors opening on approach or staying locked while rattling on failed entry) | Keeps editor-authored state swaps explicit without pushing ad-hoc branching into generic prop code. |
| **Ramp-triggered shed door** | `GardenShed` swaps authored closed/open sprites while the player foot bounds overlap the shed ramp footprint | Keeps interaction local to the prop entity without requiring bespoke map trigger scripts. |
...
| **Quest-authored NPC hints** | Main-story quests can author per-NPC dialog overrides in quest data, resolved at runtime through `QuestNpcDialogRegistry` | Keeps story-reactive NPC barks in quest content instead of hardcoded screen conditionals or NPC entity branches. |
| **Summer opening sequence** | New Game starts in `Maps/CabinBedroom`; a screen-owned `SummerIntroSequence` keeps the companion absent until the opening cottage intro completes, then publishes `StoryBeatReached` for quest completion | Aligns the first quest, companion availability, and save/load behavior without hardcoding story progression into `MomNpc` or `QuestManager`. |
| **Scripted sequence actor control** | Reusable scripted sequences operate on `IScriptControllableActor`, while autonomous patrol NPCs add `IScriptControllableNpc` for pause/resume behavior | Establishes one repeatable control surface for future narrative beats instead of sequence-specific actor hacks. |
| **Scripted sequence directives** | Repeated sequence anchors, poses, dialog payloads, and transitions live in focused definition objects backed by reusable position directives | Keeps phase logic in code while removing fragile cutscene literals from screens and map handoff glue. |
...
| **Indoor navigation graph** | `IndoorNavGraph` provides authored node/link pathfinding for indoor maps, loaded from TMX `NavNodes` and `NavLinks` object layers | Gives indoor NPCs graph-based route selection for both ambient patrol and scripted room-scale movement instead of raw waypoints, decoupled from the tile-grid collision system. |
| **Title/start screen** | `TitleScreen` displays the splash image and offers "New Game" / "Continue" menu options, checking save slot availability via `ISaveGameService` | Decouples session-start flow from `Game1` and gameplay code; Continue loads the most recent save slot. |
...
| [scripted-sequences.md](design/scripted-sequences.md) | §15 | Scripted sequence ownership, actor-control contracts, transition rules, and testing guidelines |
...
| New scripted sequence or actor-control contract | `docs/design/scripted-sequences.md` | §15 |
