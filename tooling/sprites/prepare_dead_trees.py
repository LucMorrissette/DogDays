"""
Prepare four dead tree prop sprites from a single sprite sheet JPG.
...
4) Resize so the tallest tree is 6 tiles (192 px) and the shortest is 4 tiles
   (128 px), with the others scaled proportionally by their relative heights.
   Aspect ratio is locked; nearest-neighbor resampling.
...
def main() -> None:
    ...
    # Process each quadrant: downscale if needed, remove BG, crop
    ...
    for i, img in enumerate(cropped_images):
        ...
        resized = img.resize((target_w, TARGET_HEIGHT), resample=Image.Resampling.NEAREST)

        reduced = quantize_preserve_alpha(resized, PALETTE_COLORS)

        output_path = OUTPUT_DIR / f"dead-tree{i + 1}.png"
        reduced.save(output_path)
        print(f"Tree {i + 1}: saved {output_path.name} ({reduced.size[0]}x{reduced.size[1]})")

    print("\nDone — all four dead tree sprites saved.")
